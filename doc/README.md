...
> [!NOTE]
> Port numbers and activation is configurable through `aspera.conf`.
> If facing Internet, it is recommended to front end the node API with a reverse proxy, such as `nginx`.
...
Node Users with a **docroot** (`absolute`) : it can be used to manage transfers directly.
The **docroot** is associated with the Transfer user, in `aspera.conf`, modified with `asconfigurator`.
The **docroot** is defined as a PVCL URL, which can range from local file system, to object storage (e.g. S3).
...
The **file restrictions** is associated with the Transfer user, in `aspera.conf`, modified with `asconfigurator`.
...
The storage `type` is `local`, and the `path` is relative to the storage root of the associated Primary Access Keys.
The `path` cannot be `/`, it must be a sub-folder within the Primary Access Keys storage root.

#### OAuth2 Bearer Tokens

Token-based authentication used for integration with cloud services like **Aspera on Cloud (AoC)**.
Bearer tokens are always generated associated with an Access Key.
